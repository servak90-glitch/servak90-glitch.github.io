...
### ПРИНЦИП 7: ИЗУЧЕНИЕ ИСТОРИИ ИЗ BRAIN
**ОБЯЗАТЕЛЬНО ознакомиться с содержимым папок в директории `C:\Users\Потехины\.gemini\antigravity\brain`.**

✅ **Почему это важно:**
- Там хранятся артефакты всех предыдущих задач
- Это позволяет понять контекст текущего состояния проекта
- Исключает дублирование работы и противоречащие друг другу решения

✅ **Когда проверять:**
- В самом начале каждой новой сессии или задачи
- При работе с компонентами, которые уже подвергались изменениям
...
2. Изучаю артефакты в директории brain → C:\Users\Потехины\.gemini\antigravity\brain
3. Понимаю контекст проекта
4. Для сложных задач → mcp_sequential-thinking
```
...
- [ ] Ознакомился с артефактами в директории `brain`
